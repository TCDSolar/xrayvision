...
def vis_spatial_frequency_weighting(vis, spatial_frequency_weight=1.0,
                                    uniform_weighting=True):
    """
    This function returns the spatial_frequency_weighting factor for
    each visibility in the input vis visibility bag. The weights are not
    normalized. That's left for the task that uses them
    ...
    spatial_frequency_weight : np.array, float
        ...
        dimensions of the visibilities and weights must agree.

        If uniform_weighting is True, the spatial weight will be computed
        based on sqrt(vis.u^2 + vis.v^2))
    ...
    nsfw = spatial_frequency_weight.shape[0]
    if nsfw != nvis:
        raise ValueError("Visibility bag, vis, and SPATIAL_FREQUENCY_WEIGHTs"
                         "cannot be reconciled.")


def vis_bpmap_get_spatial_weights(visin, spatial_frequency_weight=0,
...
def vis_bpmap(visin, bp_fov=80., pixel=0., uniform_weighting=False,
    ...
    pixel : float
        size of a pixel
    uniform_weighting : bool
        If true, changes subcollimator weighting from default (NATURAL) to
        UNIFORM. Default if false.
    spatial_frequency_weight : weighting for each collimator, set by
        UNIFORM_WEIGHTING if used. The number of weights should either equal
        the number of unique sub-collimators or the number of visibilities
