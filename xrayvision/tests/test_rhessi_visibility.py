...
class TestRHESSIVisibility(object):
    ...
    def test_from_map(self, N, M):
        ...
        vis = RHESSIVisibility(uv_in, vis_in)
    ...
    def test_generate_xy(self, size):
        ...
        res_even = RHESSIVisibility.generate_xy(n_even, size)
        assert np.array_equal(res_even, even)

        odd = np.arange(-32, 33)*size
        res_odd = RHESSIVisibility.generate_xy(n_odd, size)
    ...
    def test_dftmap(self, xs, ys):
        ...
        visout = RHESSIVisibility.dft_map(data, uv)
        imout = RHESSIVisibility.idft_map(visout, np.zeros((m, n)), uv)
