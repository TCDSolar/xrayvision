...
@pytest.mark.parametrize("phase_centre, pixel_size", [(0, (0.5, 1, 2, 3)),
                                                (+5.5, (0.5, 1, 2, 3)),
                                                (-5.5, (0.5, 1, 2, 3))])
...
def test_generate_uv_offset_size(phase_centre, pixel_size):
    ...
    for size in pixel_size:
        ...
        if phase_centre != 0:
            odd += 1 / phase_centre
        assert np.allclose(odd, generate_uv(m*apu.pix, phase_centre=phase_centre, pixel_size=size),
        ...
        if phase_centre != 0:
            even += 1 / phase_centre
        assert np.allclose(even, generate_uv(n*apu.pix, phase_centre=phase_centre, pixel_size=size),
                           atol=1e-8 / apu.arcsec)
...
def test_phase_centre_equivalence():
    ...
    img1 = idft_map(vis, u=u, v=v, weights=1/u.size, pixel_size=[1, 1] * apu.arcsec/apu.pix, shape=data.shape*apu.pix)
    ...
    phase_shift = (
       2 * np.pi * (5 * apu.arcsec * u + 5 * apu.arcsec * v) * apu.rad
    )
