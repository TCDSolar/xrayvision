...
def clean(dirty_map, dirty_beam, clean_beam_width=4.0, gain=0.1, thres=0.01, niter=1000):
    """
    Clean the image using Hogbom orginal method will stop when either `thres` is reached or `niter`
    iteration have been completed.

    Parameters
    ----------
    dirty_map : `numpy.ndarray`
        The dirty map to be cleaned
    dirty_beam
        The dirty beam or point spread function (PSF)
    clean_beam_width : `float`
        The width of the gaussian to convilve the model with
    gain : `float`
        The gain per loop
    thres : `float`
        A threshold val
